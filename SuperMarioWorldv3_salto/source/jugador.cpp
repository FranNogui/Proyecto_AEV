...
void Jugador::IniciarTriggerSalto(b2World* world)
{
    b2BodyDef bodyDef;
    bodyDef.type = b2_staticBody;
    bodyDef.position.Set((posicion.x + posicion.w / 2.0f) * 0.1f, (posicion.y + posicion.h / 2.0f + 0.1f) * 0.1f);
    triggerSalto = world->CreateBody(&bodyDef);
    b2PolygonShape triggerBox;
    triggerBox.SetAsBox((posicion.w / 2.0f) * 0.1f, 0.1f);
    b2FixtureDef fixtureDef;
    fixtureDef.shape = &triggerBox;
    fixtureDef.isSensor = true;
    triggerSalto->CreateFixture(&fixtureDef);
}
...
bool Jugador::TocandoTriggerSalto(b2Contact* contact) {
    b2ContactEdge* edge = cuerpoFisico->GetContactList();
    while (edge)
    {
        b2Contact* contact = edge->contact;
        if (contact->IsTouching() && contact->GetFixtureB()->GetBody() == triggerSalto)
        {
            return true;
        }
        edge = edge->next;
...
void Jugador::ManejarColisionTriggerSalto(bool empezarSaltar) {
  enAire = !empezarSaltar;
}

void Jugador::Saltar() {
    if (TocandoTriggerSalto() && !enAire)
	{
        cuerpoFisico->ApplyLinearImpulseToCenter(b2Vec2(0, -50), true);
		enAire = true;
    }
}

void Jugador::Actualizar() {
	b2ContactEdge* contacto = cuerpoFisico->GetContactList();
	while (contacto) {
    	b2Fixture* fixtureA = contacto->contact->GetFixtureA();
    	b2Fixture* fixtureB = contacto->contact->GetFixtureB();
    	if ((fixtureA == cuerpoFisico->GetFixtureList() && fixtureB == triggerSalto->GetFixtureList()) ||
        	(fixtureB == cuerpoFisico->GetFixtureList() && fixtureA == triggerSalto->GetFixtureList())) {
        	enAire = false;
        	break;
    	}
